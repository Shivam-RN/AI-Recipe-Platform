...
// Get recipes generated by current user
export async function getGeneratedRecipes() {
  try {
    ...
    // Fetch recipes where author = current user
    const response = await fetch(
      `${STRAPI_URL}/api/recipes?filters[author][id][$eq]=${user.id}&populate=*&sort=createdAt:desc`,
      {
        headers: {
          Authorization: `Bearer ${STRAPI_API_TOKEN}`,
        },
    ...
    throw new Error(error.message || "Failed to load generated recipes");
  }
}