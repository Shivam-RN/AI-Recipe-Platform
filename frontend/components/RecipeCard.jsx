...
export default function RecipeCard({ recipe, variant = "default" }) {
  ...
  if (variant === "grid") {
    return (
      <Link href={data.href}>
        <Card className="rounded-none overflow-hidden border-stone-200 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 cursor-pointer group pt-0 lex flex-col h-full">
          {/* Image */}
          {data.showImage ? (
